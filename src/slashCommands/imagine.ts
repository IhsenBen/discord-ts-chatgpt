...
const ImagineCommand: SlashCommand = {
    ...
    execute: interaction => {
        const prompt = interaction.options.get("prompt");
               ...
               return generateImage(`${prompt}`).then(url => {
                    const embed = new EmbedBuilder()
                        .setImage(url)
                        .setDescription(`Here is your generated image for **"${prompt?.value}"**:`)
                    interaction.editReply({ embeds: [embed], content: '<:requisitos:994730420606341240>' })
                })
